 ...
 */
public class GmRefreshLayout extends ViewGroup {
    ...
    private int headerHeight = 0;       // header高度
    ...
    public GmRefreshLayout(Context context) {
        this(context, null);
    }

    public GmRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);

        touchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
        autoScroll = new AutoScroll();
        setRefreshHeader(new GmDefaultHeaderView(getContext()));
    }
    ...
    private void moveSpinner(float diff) {
        ...
        // 别忘了回调header的位置改变方法。
        if (refreshHeader instanceof OnRefreshHeader) {
...
//        Toast.makeText(getContext(), state.toString(), Toast.LENGTH_SHORT).show();
        OnRefreshHeader refreshHeader = this.refreshHeader instanceof OnRefreshHeader ? ((OnRefreshHeader) this.refreshHeader) : null;
    ...
    public void startRefresh() {
        if (isTouch || state == State.LOADING || state == State.COMPLETE) {
            return;
        }
            ...
            public void run() {
                ...
                for (int i = 0; i < 20; i++) {
                    ...
                    postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            dispatchTouchEvent(MotionEvent.obtain(0, 0, MotionEvent.ACTION_MOVE, 0, count * finalI, 0));
                        }
                    }, 10 * i);
                }
                ...
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        dispatchTouchEvent(MotionEvent.obtain(0, 0, MotionEvent.ACTION_UP, 0, count * 20, 0));
                    }
                }, 210);
            }
        }, isFirst ? 200 : 0);
